  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/support/inquiries`, {
        ...
        body: JSON.stringify({ category, title, body, email: email.trim(), user_id: userId }),
      });
      if (!res.ok) {
      ...
      <div className="flex flex-col gap-6 bg-white p-8 rounded-3xl shadow-sm border border-blue-500/10">
        ...
        <Field label="문의 내용">
          <textarea
            ...
            className="w-full bg-white outline-none resize-none text-sm leading-relaxed placeholder-gray-400 font-medium"
            style={{
              color: "#1a1f27",
              minHeight: 160,
        ...
        >
          {submitting ? (
            <>
              <div className="w-5 h-5 rounded-full border-2 animate-spin" style={{ borderColor: "rgba(255,255,255,0.3)", borderTopColor: "transparent" }} />
              전송 중…
            </>
          ) : (
            <>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-5" style={{ scrollbarWidth: "none", background: "#ffffff" }}>
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            {msg.role === "bot" ? (
              ...
              <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 self-end bg-gray-100">
        ...
        {typing && (
          <div className="flex gap-3">
            ...
            <div className="flex items-center gap-1.5 px-5 py-4 rounded-2xl" style={{ background: "#f1f5f9", borderBottomLeftRadius: 6 }}>
              {[0, 1, 2].map((i) => (
      ...
      >
        <input
          ...
          className="flex-1 bg-gray-50 rounded-xl px-4 py-3 outline-none text-sm placeholder-gray-400 font-medium transition-all border border-transparent"
          style={{ color: "#1a1f27" }}
        />
        <button
...
function FieldInput({ value, onChange, placeholder, type = "text", maxLength }) {
  return (
    <input
      ...
      maxLength={maxLength}
      className="w-full bg-white outline-none text-sm placeholder-gray-400 font-medium transition-all"
      style={{
        border: "1px solid rgba(47, 107, 251, 0.15)",
        borderRadius: "14px",
        padding: "14px 16px",
