...
export default function AnnouncementEdit() {
  ...
  // 💡 기존 글 데이터 불러오기 및 카테고리 분리
  useEffect(() => {
    axios.get(`${BACKEND_URL}/api/announcements/${id}`)
      .then(res => {
        const fetchedTitle = res.data.title || '';
        
        // 정규식으로 "[카테고리] 진짜제목" 형태를 분리합니다.
        const match = fetchedTitle.match(/^\[(.*?)\]\s*(.*)$/);
        
        if (match && CATEGORY_META[match[1]]) {
          setCategory(match[1]); // 추출한 카테고리 설정
          setTitle(match[2]);    // 괄호를 뺀 순수 제목만 설정
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 수정할 때도 선택된 카테고리를 다시 묶어서 전송합니다.
      const formattedTitle = `[${category}] ${title}`;

      await axios.put(
        `${BACKEND_URL}/api/announcements/${id}`,
        { title: formattedTitle, content: body },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setSubmitted(true);
      setTimeout(() => navigate(`/announcements/${id}`), 1800);
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6" style={{ background: '#f9fafb' }}>
        <div className="w-20 h-20 rounded-3xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#4a90e2,#6c63ff)', boxShadow: '0 0 60px rgba(74,144,226,0.2)' }}>
          <Send className="w-9 h-9 text-white" />
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-gray-900 mb-2">공지사항이 수정되었습니다</p>
          <p className="text-sm" style={{ color: '#6b7280' }}>잠시 후 상세 페이지로 이동합니다</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#ffffff', color: '#111827', fontFamily: "'Inter', sans-serif" }}>
      {/* ── Top bar ── */}
      <header className="flex items-center justify-between px-8 py-4 flex-shrink-0" style={{ borderBottom: '1px solid #e5e7eb' }}>
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            style={{
              background: canSubmit && !isSubmitting ? 'linear-gradient(135deg,#4a90e2,#6c63ff)' : '#f3f4f6',
      ...
      <div className="flex flex-1 min-h-0">
          ...
          {preview ? (
            <div className="flex-1 flex flex-col max-w-3xl">
              ...
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={CATEGORY_META[category]}>
                  {category}
                </span>
                {isPinned && (
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1" style={{ background: 'rgba(234,88,12,0.1)', color: '#ea580c' }}>
              ...
              <div className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: '#374151' }}>
            ...
            <div className="flex-1 flex flex-col max-w-3xl">
              ...
              <div className="mb-0" style={{ height: 1, background: '#e5e7eb' }} />
              ...
              <textarea
                ...
                style={{ color: '#374151', minHeight: 320 }}
              />
        ...
        {/* ── Right sidebar ── */}
        <aside className="w-64 flex-shrink-0 px-6 py-8 flex flex-col gap-6 bg-gray-50" style={{ borderLeft: '1px solid #e5e7eb' }}>
          <div>
            ...
            <div className="relative">
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${CATEGORY_META[category].color}40`,
              ...
              {catOpen && !isSubmitting && (
                <div className="absolute top-full left-0 right-0 mt-1 rounded-xl overflow-hidden z-10 bg-white shadow-lg" style={{ border: '1px solid #e5e7eb' }}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <Toggle icon={<Pin className="w-4 h-4" />} label="상단 고정" desc="목록 최상단에 표시" value={isPinned} onChange={setIsPinned} accent="#ea580c" disabled={isSubmitting} />
              <Toggle icon={<Bell className="w-4 h-4" />} label="푸시 알림" desc="모든 사용자에게 전송" value={isPush} onChange={setIsPush} accent="#2563eb" disabled={isSubmitting} />
            </div>
          </div>

          {!canSubmit && (title.length > 0 || body.length > 0) && (
            <div className="flex items-start gap-2 px-3 py-3 rounded-xl text-xs bg-orange-50" style={{ color: '#ea580c', border: '1px solid rgba(234,88,12,0.2)' }}>
              <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <span className="font-medium">제목과 내용을 모두 입력해야 수정할 수 있습니다.</span>
            </div>
          )}

          <div className="mt-auto pt-6 border-t border-gray-200">
            <p className="text-xs font-bold mb-2 uppercase tracking-[0.1em]" style={{ color: '#9ca3af' }}>작성 가이드</p>
            {['제목은 80자 이내로 작성하세요', '**굵게**, _기울임_ 문법 지원', '• 로 목록 항목을 만들 수 있어요'].map((g) => (
              <p key={g} className="text-xs font-medium leading-relaxed mb-1" style={{ color: '#9ca3af' }}>· {g}</p>
            ))}
          </div>
        </aside>
      </div>
...
function Toggle({ icon, label, desc, value, onChange, accent, disabled }) {
  return (
    ...
    >
      ...
      <div className="w-8 h-4 rounded-full flex-shrink-0 mt-0.5 relative transition-all duration-200" style={{ background: value ? accent : '#d1d5db' }}>
        <div className="absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all duration-200" style={{ left: value ? '17px' : '2px', boxShadow: '0 1px 2px rgba(0,0,0,0.2)' }} />
      </div>
    </button>
  );
}