...
import {
  ...
  Settings, ChevronRight, ChevronLeft, Sparkles, Clock, Sun, Moon, Send, X, Volume2,
...
function ControlBtn({ active, onClick, icon, danger = false, label }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1 group">
      <div className="w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-200 group-hover:scale-105 shadow-sm"
        style={{
...
export default function CoffeeChatRoom() {
  ...
  useEffect(() => {
    ...
    if (!id) return;
    axios.get(`${BACKEND_URL}/api/booking/detail/${chatId}`).then(res => setBooking(res.data)).catch(console.error);
    axios.get(`${BACKEND_URL}/api/chat-session/${chatId}`).then(res => setSession(res.data)).catch(console.error);
  }, [chatId]);

  useEffect(() => { const t = setInterval(() => setDuration(p => p + 1), 1000); return () => clearInterval(t); }, []);
  useEffect(() => { if (chatScrollRef.current) chatScrollRef.current.scrollTop = chatScrollRef.current.scrollHeight; }, [chatMessages]);
  useEffect(() => { if (llmScrollRef.current) llmScrollRef.current.scrollTop = llmScrollRef.current.scrollHeight; }, [llmMessages, llmBuffer]);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'chunk') { setLlmStreaming(true); setLlmBuffer(prev => prev + data.text); }
          ...
          if (Array.isArray(data.questions) && data.questions.length > 0) setRecommendedQuestions(data.questions);
        }
      } catch (e) {}
    };
    return () => ws.close();
  ...
  useEffect(() => {
    if (!userId || !chatId) return;
    const countdown = setInterval(() => setNextRefreshIn(prev => prev <= 1 ? RECOMMEND_INTERVAL_MS / 1000 : prev - 1), 1000);
    ...
    return () => { clearInterval(countdown); clearInterval(interval); };
  }, [userId, chatId, booking]);

  const handleManualRefresh = useCallback(() => {
    const buffer = sttBufferRef.current.trim();
    if (!buffer || llmWsRef.current?.readyState !== WebSocket.OPEN) return;
    setIsGeneratingQuestions(true);
  ...
  const handleLlmSubmit = (e) => {
    ...
    if (llmWsRef.current?.readyState === WebSocket.OPEN) {
      setLlmStreaming(true); setLlmBuffer('');
  ...
  const handleToggleVideo = useCallback(() => {
    const newOff = !isVideoOff; setIsVideoOff(newOff);
    localVideoRef?.current?.srcObject?.getVideoTracks().forEach(t => { t.enabled = !newOff; });
  ...
  const handleEndCall = async () => {
    ...
    try { if (session?.session_id) await axios.post(`${BACKEND_URL}/api/chat-session/end/${session.session_id}`); } catch {}
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden transition-colors duration-300"
      ...
      {/* ── 헤더 ── */}
      <header className="flex-shrink-0 relative z-10 flex items-center justify-between px-6 py-3"
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
            className="p-2 rounded-full hover:bg-black/10 transition-colors" style={{ color: 'var(--text-main)' }}>
          ...
          {/* 헤더 타이머 — data-tutorial */}
          <div data-tutorial="header-timer"
      ...
      <div className="flex-1 min-h-0 flex flex-col px-6 pt-4 pb-4 gap-3">
        <div className="flex-1 min-h-0 flex gap-4">
          ...
          <div className="flex-1 min-h-0 flex flex-col gap-3">
            ...
            {/* 비디오 영역 — data-tutorial */}
            <div data-tutorial="video-area" className="flex-1 min-h-0 flex gap-4">

              {/* 내 화면 */}
              <div className="flex-1 relative rounded-3xl overflow-hidden flex items-center justify-center shadow-lg"
                style={{ background: 'var(--panel-bg)', border: '1px solid var(--panel-border)', backdropFilter: 'blur(20px)' }}>
                <video ref={localVideoRef} autoPlay playsInline muted
                  className="absolute inset-0 w-full h-full object-cover" style={{ display: isVideoOff ? 'none' : 'block' }} />
                {isVideoOff && (
              ...
              <div className="flex-1 relative rounded-3xl overflow-hidden flex items-center justify-center shadow-lg"
                ...
                <video ref={remoteVideoRef} autoPlay playsInline
                  onPlay={handleRemoteVideoPlay} onEmptied={handleRemoteVideoEmptied}
                  className="absolute inset-0 w-full h-full object-cover" style={{ display: isRemoteConnected ? 'block' : 'none' }} />
                {!isRemoteConnected && (
                  <div className="flex flex-col items-center gap-3 z-10">
            ...
            <div data-tutorial="stt-area">
              {sttMode !== 'hidden' && (
                <div className={`rounded-2xl p-4 flex flex-col shadow-md transition-all duration-300 overflow-hidden ${sttMode === 'expanded' ? 'flex-1 min-h-0' : ''}`}
                  ...
                  <div className="flex items-center justify-between mb-2 flex-shrink-0">
                    ...
                    <button onClick={cycleSttMode}
                      className="text-xs px-2 py-0.5 rounded-full transition-colors hover:bg-blue-500/10"
              ...
              {sttMode === 'hidden' && (
                <button onClick={cycleSttMode}
                  className="flex items-center gap-2 px-4 py-2 rounded-2xl text-xs transition-colors hover:opacity-80 shadow-sm"
                  style={{ background: 'var(--panel-bg)', border: '1px solid var(--panel-border)', color: 'var(--text-muted)' }}>
          ...
          {/* ── 우측 탭 패널 — data-tutorial ── */}
          <div data-tutorial="tab-panel" className="w-80 flex-shrink-0 min-h-0 flex flex-col relative">
            <div className="flex-1 min-h-0 flex flex-col rounded-2xl shadow-md overflow-hidden"
              style={{ background: 'var(--panel-bg)', border: '1px solid var(--panel-border)' }}>
                ...
                {activeTab === 'ai' && (
                  <>
                    <div className="flex-shrink-0 flex items-center justify-between mb-3">
                      <span className="text-xs font-semibold text-amber-500">AI 추천 질문</span>
                      <div className="flex items-center gap-2">
                        ...
                        {isGeneratingQuestions
                          ...
                          : <button onClick={handleManualRefresh} disabled={sttBufferRef.current.length < MIN_BUFFER_LENGTH}
                    ...
                    <div className="flex-shrink-0 mt-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{Math.min(100, Math.round((1 - nextRefreshIn / (RECOMMEND_INTERVAL_MS / 1000)) * 100))}%</span>
                      </div>
                ...
                {activeTab === 'llm' && (
                  <>
                    ...
                    <form onSubmit={handleLlmSubmit} className="flex-shrink-0 flex items-center gap-2 relative">
                      <input type="text" value={llmInput} onChange={e => setLlmInput(e.target.value)}
                        ...
                        style={{ color: 'var(--text-main)' }} />
                      <button type="submit" disabled={llmStreaming || !llmInput.trim()}
            ...
            <div className="absolute inset-0 rounded-2xl flex flex-col transition-all duration-300 ease-in-out"
              style={{
                background: 'var(--chat-overlay)', border: '1px solid var(--panel-border)',
              ...
              <div ref={chatScrollRef} className="flex-1 min-h-0 overflow-y-auto px-4 py-3 flex flex-col gap-2 custom-scrollbar">
                {chatMessages.map((m, i) => (
                  <div key={i} className={`flex flex-col max-w-[85%] flex-shrink-0 ${m.sender === 'me' ? 'self-end items-end' : m.sender === 'system' ? 'self-center' : 'self-start items-start'}`}>
                    ...
                    <div className={`p-2.5 rounded-xl text-xs ${m.sender === 'me' ? 'bg-indigo-500 text-white rounded-tr-sm' : m.sender === 'system' ? 'bg-transparent text-gray-400 text-[10px]' : 'bg-black/10 rounded-tl-sm'}`}
        ...
        <div className="flex-shrink-0 flex items-center justify-center">
          <div data-tutorial="controls"
            ...
            <ControlBtn active={false} onClick={() => setShowTutorial(true)} icon={<HelpCircle className="w-5 h-5" />} label="가이드" />
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-96 rounded-2xl p-6 shadow-2xl" style={{ background: 'var(--panel-bg)', border: '1px solid var(--panel-border)' }}>
            ...
            <div className="space-y-4">
              {[['마이크', <Mic className="w-4 h-4" />, ['기본 마이크 (내장 마이크)', '외부 USB 마이크']],
                ['스피커', <Volume2 className="w-4 h-4" />, ['기본 스피커', '블루투스 헤드폰']],
                ['카메라', <Video className="w-4 h-4" />, ['FaceTime HD Camera', '외부 웹캠']]].map(([name, icon, opts]) => (
                <div key={name}>
                  <label className="text-sm font-semibold mb-2 flex items-center gap-2" style={{ color: 'var(--text-main)' }}>{icon} {name}</label>
